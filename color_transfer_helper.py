...
class ColorCheckerLocation:
    ...
    def build_from_pattern(pattern: np.ndarray) -> 'ColorCheckerLocation':
        return ColorCheckerLocation((0, 0), (0, pattern.shape[0]), (pattern.shape[1], 0))

    def to_np_array(self) -> np.ndarray:
        return np.float32((self.top_left, self.top_right, self.bottom_left))

    def __repr__(self):
        return f'TopLeft: {self.top_left}, TopRight: {self.top_right}, Bottom_Left: {self.bottom_left}'
